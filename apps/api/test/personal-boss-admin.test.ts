...
describe("personal boss prompt administration", () => {
  ...
  it("reconstructs current persona and the latest real chat prompt, then audits without raw text", async () => {
    ...
    await store.updateAiPromptSettings({ translation: previousPrompts.translation, translationReplyStyles: previousPrompts.translationReplyStyles, coaching: previousPrompts.coaching, onboarding: { ...previousPrompts.onboarding, personaGeneration: "원문 테스트 페르소나 생성 지침" } });
    ...
    try {
      ...
      await store.updateAiPromptSettings({ translation: previousPrompts.translation, translationReplyStyles: previousPrompts.translationReplyStyles, coaching: previousPrompts.coaching, onboarding: previousPrompts.onboarding });
