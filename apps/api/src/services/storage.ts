...
class StorageService {
  ...
  async verify(path:string,size:number,contentType:string){if(!this.client)return true;const parts=path.split("/");const name=parts.pop()!;const folder=parts.join("/");const {data,error}=await this.client.storage.from(env.SUPABASE_STORAGE_BUCKET).list(folder,{search:name,limit:2});if(error)throw error;const object=data.find(item=>item.name===name);const actualSize=Number(object?.metadata?.size??-1);const actualType=String(object?.metadata?.mimetype??object?.metadata?.contentType??"");return Boolean(object&&actualSize===size&&(!actualType||actualType===contentType));}
