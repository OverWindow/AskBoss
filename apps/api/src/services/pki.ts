...
export function calculatePki(evidence:EvidenceRecord[],persona:BossPersona):PkiBreakdown {
  ...
  const freshness=OBSERVATION_CATEGORIES.map((category)=>{ const dates=observations.filter((o)=>o.category===category).map((o)=>o.observedAt??o.fallbackDate).filter(Boolean).sort().reverse(); return dates[0]?Math.exp(-daysSince(dates[0])/180):0; }).reduce((a,b)=>a+b,0)/5;
