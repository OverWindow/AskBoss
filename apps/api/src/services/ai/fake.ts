...
export class FakeAiService implements AiService {
  ...
  async chatWithBoss(input:any) { const message=input.message as string; if(message.includes("늦")) return "현재 진행 상황부터 간단히 정리해서 알려주세요. 남은 일정도 같이 봅시다."; return "좋아요. 결론과 다음에 할 일을 먼저 말해주면 더 빨리 판단할 수 있어요."; }
