...
export class MindlogicAiService implements AiService {
  ...
  private async structured<T>(model:string,prompt:string,schema:ZodType<T>,signal?:AbortSignal,system?:string){return this.validateOrRepair(model,await this.text(model,prompt,signal,system),schema,signal);}
  ...
  generateMonologue(input:any){const {basePrompt,...context}=input;return this.text(env.AI_PRIMARY_MODEL,monologuePrompt(context),undefined,input.boss?.scope==="SESSION"?personalBossSystemPrompt(basePrompt):undefined);}
