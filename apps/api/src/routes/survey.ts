...
export const surveyRoutes:FastifyPluginAsync=async(app)=>{
  app.post("/bosses/:bossId/survey/generate",{config:{rateLimit:{max:20,timeWindow:"1 minute"}}},async(request)=>{const s=await requireSession(request);const [boss,prompts]=await Promise.all([store.getBoss(s.id,(request.params as any).bossId),store.getAiPromptSettings()]);if(!boss||boss.scope==="GLOBAL")throw new HttpError(404,"상사를 찾을 수 없습니다.");return {questions:await ai.generateSurvey(boss,prompts.onboarding.surveyGeneration)};});
