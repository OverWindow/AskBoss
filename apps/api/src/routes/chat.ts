...
export const chatRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post("/bosses/:bossId/chat", { config: { rateLimit: { max: 20, timeWindow: "1 minute" } } }, async (request, reply) => {
    ...
    try {
      let content = "";
      for await (const chunk of ai.streamChatWithBoss({ profile, boss, summary: thread.conversationSummary, messages: previousMessages, message: body.message }, controller.signal)) {
        if (!chunk) continue;
        content += chunk;
