...
export const hrRoutes:FastifyPluginAsync=async(app)=>{
  ...
  app.get("/hr/usage/by-time",async()=>({data:(await data()).byTime}));
  app.get("/hr/topics",async()=>({data:(await data()).topics}));
  app.get("/hr/summary",async()=>{const aggregates=await data();return {summary:await ai.generateHrSummary(aggregates)};});
