...
export const bossRoutes:FastifyPluginAsync=async(app)=>{
  ...
  app.delete("/bosses/:bossId",async(request,reply)=>{const s=await requireSession(request);const bossId=(request.params as any).bossId;const evidence=await store.listEvidence(s.id,bossId);await storage.remove(evidence.flatMap(item=>item.storagePath?[item.storagePath]:[]));await store.deleteBoss(s.id,bossId);return reply.code(204).send();});
};
