...
export function Sidebar(){
  ...
  return <aside className={`sidebar ${ui.mobileNavOpen?"is-mobile-open":""}`} aria-label="주요 메뉴">
    ...
    <nav className="boss-nav"><div className="nav-section-label hide-collapsed">상사</div>{bosses.map((boss,index)=><button key={boss.id} data-tutorial={boss.scope==="GLOBAL"?"global-boss":undefined} className={`boss-nav-item ${ui.selectedBossId===boss.id||(!ui.selectedBossId&&index===0)?"is-active":""}`} onClick={()=>{ui.set({selectedBossId:boss.id,mobileNavOpen:false});navigate("/");}} title={boss.alias}><img className="mini-avatar" src={`/avatars/${boss.avatarKey}.png`} alt=""/><span className="hide-collapsed">{boss.alias}</span></button>)}<div className="nav-divider"/><Link to="/boss/new" data-tutorial="add-boss" className="boss-nav-item add-boss" onClick={()=>ui.set({mobileNavOpen:false})} title="상사 추가"><Plus size={18}/><span className="hide-collapsed">상사 추가</span></Link></nav>
    <div className="sidebar-bottom"><Link className="nav-link" to="/hr-demo" title="HR Demo"><BarChart3 size={19}/><span className="hide-collapsed">HR Demo</span></Link><button className="sidebar-user" onClick={()=>ui.set({settingsOpen:!ui.settingsOpen})} title={profile?`@${profile.handle}`:"사용자 설정"}><UserRound size={19}/><span className="hide-collapsed">{profile?`@${profile.handle}`:"사용자 설정"}</span></button>{ui.settingsOpen&&<div className="profile-menu"><Link to="/settings" onClick={()=>ui.set({settingsOpen:false})}>내 정보 · 상사 관리</Link><button onClick={()=>{try{localStorage.removeItem(TUTORIAL_STORAGE_KEY);}catch{/* Storage may be unavailable. */}ui.set({tutorialOpen:true,settingsOpen:false});navigate("/");}}>Tutorial 다시 보기</button><button className="danger-button" onClick={reset}>Session 데이터 초기화</button></div>}</div>
  </aside>;
}
